...
def train_worker(img_path, verbose, class_dir):
    while True:
...
def train(train_dir, model_save_path=None, n_neighbors=None, knn_algo='ball_tree', verbose=False):
    ...
    for class_dir in listdir_nohidden(train_dir):
        if not os.path.isdir(os.path.join(train_dir, class_dir)):
            continue

        # Loop through each training image for the current person
        for img_path in image_files_in_folder(os.path.join(train_dir, class_dir)):
            tasks.append((img_path, verbose, class_dir))
            # image = face_recognition.load_image_file(img_path)
            # face_bounding_boxes = face_recognition.face_locations(image)
            #
            # if len(face_bounding_boxes) != 1:
    ...
    for (class_dir, encoding) in pool.starmap(train_worker, tasks):
        if (class_dir == -1):
            continue
        X.append(encoding)
        y.append(class_dir)
...
def test(weights):
    ...
    for image_dir in listdir_nohidden(test_dir):
        ...
        #         test_result.append(name==label)
        # print("- Found {} at ({}, {})".format(name, left, top))
        # Display results overlaid on an image
    for (label, predictions) in pool.starmap(test_worker, tasks):
        # if predictions:
        test_result.append(label == predictions[0][0])
        result_labels.append(predictions[0][0])
        # else:
